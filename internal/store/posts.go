...
type PostWithMedata struct {
	Post
	CommentCount int `json:"comments_count"`
}

func (s *PostStore) GetUserFeed(ctx context.Context, userID int64, fq PaginationFeedQuery) ([]PostWithMedata, error) {
	query := `
		select 
	...
	var feed []PostWithMedata

	for rows.Next() {
		var p PostWithMedata
